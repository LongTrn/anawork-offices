...
import { OfficeDetailBody, } from "../index"
...
// forwardRef(function Body(props, ref) {
...
const Footer = forwardRef(function Footer({ state, }, ref) {
	...
	const onUpdate = async (state, id, index = 1, size = 10) => {
		
		// // ref.current.submit(id, index, size);
	...
	return (
		<div className="modal-footer office-detail__footer">
			...
			<button className="btn shadow-none modal-footer__buttons modal-footer__buttons--left" onClick={() => onUpdate(state, state.id, index, size)}><span className="modal-footer__buttons__text--left">Cập nhật</span></button>
		</div>
	)
})
...
export default function OfficeDetail(props) {
	...
	useEffect(() => {
		console.log("bodyRef", bodyRef.current.state())
		console.log("footerRef", footerRef.current.state())
		setState(bodyRef.current.state())
	}, [ bodyRef, footerRef])
